...
st.set_page_config(
    page_title="CAS Sourcing MVP v6",
    page_icon="🧪",
    layout="wide",
)

st.title("🧪 CAS Sourcing & Procurement Intelligence MVP v6")
st.caption(
    "CAS input → supplier discovery → visible pricing extraction → normalized price → bulk estimate → shortlist. "
    "v6 preserves prior modes and adds stricter product-page targeting, embedded-script price extraction, CAS-neighborhood extraction, and duplicate cleanup."
)
...
if run_search:
    ...
    st.download_button(
        ...
        data=csv,
        file_name=f"cas_supplier_shortlist_v5_{cas_number.replace('-', '_')}.csv",
        mime="text/csv",
    )
else:
